...
//---------------------------------------------------
		...
		function module() {
			...
			function exports(_selection) {
				_selection.each(function(param) {
					...
					config = angular.extend(config, param._option);
...
//				//全局zoom行为定义
						...
						// ************** 预定义绘图区域	 *****************
						var flag = d3.select($node).select(".dtree-svg");
						if(flag[0][0] == null) {
							//divs是浮动文字层，负责显示文字信息
						...
						//全局效果绑定
						flag = d3.select($node).select(".dtree-svg");
...
//					svg.on("mousemove.zoom", null);
							flag.on("dblclick.zoom", null);
							flag.on("touchstart.zoom", null);
							flag.on("wheel.zoom", null);
							flag.on("mousewheel.zoom", null);
							flag.on("MozMousePixelScroll.zoom", null);
						}
